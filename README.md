# Your First NFT，献给 Web3 新手的第一个 NFT


进入 Web3 第一步，从拥有第一张 NFT 开始。

NFT 的一小步，Web3  的一大步。

Your First NFT（你的第一张 NFT）， 让 Web3 新手获得第一张免费 NFT，以此作为进入 Web3 新世界的门票。

## 介绍

新人理解 Web3 的第一须知： Web3 的概念并没有那么重要。

所有概念都是定义出来的，你可以通过使用它，来理解它，从而定义它（use it，know it，define it），包括 Web3 。

一千人有一千个哈姆雷特，你可以通过局部理解，通过感性认知，通过具体操作，来形成自己对 Web3 的理解。

你一开始不必掌握 Web3 的宏大图景，就像你父母不需要知道 Web 2.0 才能使用微信红包，就像你父母不需要知道什么是社交网络，也可以使用微信朋友圈。

- 你可以先创建一个钱包来先进入 Web3
- 你可以先拥有第一张 NFT 来先使用 Web3
- 你可以先通过 Mint 第一张 NFT 来理解 Web3

**本项目试图解决问题**

- 多数人认为 Web3 就是炒币追小图片搞钱
> 但其实 Web3 是一种新的数字文明。
- 多数人怕在 Web3 被骗，不敢碰加密钱包
> 如果你的钱包只有一块钱，一块钱买不了吃亏，买不了上当。
- 多数人看不懂 Web3 ，想轻度了解一下，但不知道如何入场
> 先拥有一个 NFT，搞个门票进场看看。
- 多数项目 NFT 要钱，眼花缭乱
> 恭喜你，本项目 NFT Mint 费用为 0



​
- Your First NFT 官网：[https://yourfirstnft.xyz/](https://yourfirstnft.xyz/)
- Github 项目地址：[https://github.com/yourfirstnft/YourFirstNFT](https://github.com/yourfirstnft/YourFirstNFT)
- Opensea 地址：[https://opensea.io/collection/yourfirstnft-v3](https://opensea.io/collection/yourfirstnft-v3)
- Discord 社区地址：[https://discord.gg/b7KRccfYN3](https://discord.gg/b7KRccfYN3)


> **如果此项目对你有用，麻烦点个星星，谢谢。**

![Your First NFT-tuya-tuya.png](https://s2.loli.net/2022/02/15/ODfyE3PHt4WQA7U.png)
[
](https://yourfirstnft.xyz/)

​
## 说明

本项目铸造费用为 0 。

本项目努力将交易手续费降到最低，目前使用代币 Polygon Matic 铸造，手续费至少需要 0.005 个 Matic（人民币约 0.05 元）。

该手续费并非本项目收取。

注意：本项目仅供 Web3 新手学习使用，它基本不值钱，因为它是无限量供应的，学习意义大于金钱意义。

​
## 免费 Mint 步骤

以下步骤操作完成后，你会了解 Web3 的基础知识：数字钱包、虚拟货币、交易市场、数字资产等。

- **1. 拥有一个 metamask 钱包**
> 免费注册教程：[https://github.com/cnfeat/YourFirstNFT/blob/main/MetaMask.md](https://github.com/cnfeat/YourFirstNFT/blob/main/MetaMask.md)
-  2.拥有少量 Matic 代币
> 代币购买教程（待上线）
-  3.使用  metamask 钱包，切换至 Polygon 网络，登录官网铸造
> 官网：https://yourfirstnft.xyz/
> 
> 铸造教程（待上线）
-  4.查看 NFT
> 登录你的 Opensea 账户查看：[https://opensea.io/account](https://opensea.io/account)
> 
> 铸造完成估计等 5 分钟才能查看，实际以 Opensea 的 Polygon 传输网速为准 

说明：

- 疑难解决，加入社区解决：[https://discord.gg/H5QtSfKu](https://discord.gg/H5QtSfKu)
- 如果你在「步骤 2」存在问题，可以直接填写[表单登记地址](http://hardwaylab.mikecrm.com/LDY8lx4)，我们人工空投给你，[NFT 查看教程](https://github.com/yourfirstnft/YourFirstNFT/blob/main/OpenseaNFT.md)

​
## 项目愿景

成为 Web3 更健康更长久的项目。

本项目鼓励你成为 Web3  早起参与者和建造者，鼓励你去做那些 Web3 中更健康更长久的事情。

以下是我们认为 Web3 中更健康更长久的事情。 

- 鼓励你使用 [Mirror](https://mirror.xyz/) 创建你的第一篇 Web3 文章，发表你的 Web3 见解与实践 
- 鼓励你使用 [RSS3](https://rss3.bio/)，创造你的 Web3 主页，发布你的 Web3 作品
- 鼓励你加入 [NextDAO](https://mirror.xyz/nextdao.eth/dLGSM1BmvOh84OxGVljJ_lsdM804q5pFRdaYTlKqO9Q)，理解什么是去中心化，发挥你的技能和优势加入对应的工作组
- 鼓励你自学摸索 Web3 基本规则与运行原理，积累经验和知识，成为可能自我，为未来职业转型做好准备

**本项目优点**
- 没有白名单，不必肝，无需抢，不用创作不用表演
- 铸造免费，手续费几乎市场最低

​
## Web3 入门必读

- [Messari年度报告](https://w3hitchhiker.mirror.xyz/znaPSL__AdRETrJoWmr-uxSkNeWJ3HQAkCJrfxmzfnA)
- [a16z合伙人对话Paradigm合伙人：我们第一次把经济系统内嵌到了互联网里](https://mirror.xyz/0x0E58bB9795a9D0F065e3a8Cc2aed2A63D6977d8A/X1MKA84sZzsw9zMfgE-Fj89Jcj8Hg43anLknCsM96dI)
- [关于Web3: 退休这两年我错过的技术趋势 — 郭宇](https://guoyu.mirror.xyz/f2yLu59TJ4u07FQ13UmiN4-cLELv3g-qQirY7YF_Qyw)

> 让你对 Web3 产生好奇的文章是哪篇，欢迎推荐。

​
## 路线图 Roadmap 

**2022 Q2** 完成合约发布，项目网站上线
**2022 Q3** 让本项目更健康更长久
**2022 Q4** 本项目永不跑路

​
## 项目成员
​
- [cnfeat.eth](https://okjk.co/rsbOXi)，[RSS3主页](https://cnfeat.eth.cheers.bio/ )，微信：hardwaylab
- [nick](https://www.devzhangh.com/)
​

## NFT 说明
​
- 建议不要购买或售卖此 NFT，避免造成不必要的误解
- NFT 主图案为登上月球第二人奥尔德林的脚印，文字为 hello world（你好，世界），为多数编程教程指引新手打印出第一句话
​

## 最后
​

愿你的 Web3 道路漫长惊奇，充满冒险。
愿你的 Web3 精神生生不息，繁荣昌盛。


## 源起

更新：[写在人工空投 800 张免费 NFT 之后 — 笨方法实验室](https://mirror.xyz/cnfeat.eth/df1gyICFbmafbvaaGntjMLsnRYkN2F-mUtrLYC16B7Y)


我进入 Web3，纯粹是年度关键词[「好奇」](http://newsletter.hardwaylab.com/issues/2022-973735)的驱动。
​

刚进入 Web3 ，我对它第一印象就是，做什么都需要钱，买个币要钱，买个图片要钱，免费的也要手续费……
​

当时就在想，能不能让我有动机先去注册一个钱包，让我有理由去试一试，去看一看，然后再考虑下一步怎么走？
​

后来接触到了 NFT，发现到处谈论的是白名单。
​

怎么去获取白名单呢？
​

你要满足大量的条件，例如社区在线聊天时长等级、邀请社区人头、混社区看一波又一波的通告、参加解密游戏、参加项目创意比赛、去学英语学画画、学做视频、学写项目故事、撰写精美简历、申请成为社区管理员等。
​

**这种为获取白名单而付出的大量的时间、精力、金钱的升级内卷的过程，戏称为「肝白」。**
**​**

肝白对于 Web3 新手，也是有好处的。
​

你可以深入了解 NFT 项目起源故事，创始团队的初心及运营能力，白名单的配置是否合理，遇到各种玩家的冲突如何解决。你也可以熟悉加密市场是怎样运作的，项目方哪些动作是对用户有价值的，哪些人真正赚到了钱，知道如何判断加密项目的真正价值。
​

但即使肝到了白，你还是要买，你只是获得某个 NFT 项目的优先购买权而已。


回头一想，我是 Web3 新手呀，Web3 规则啥的我都还不懂，为什么一开始就要去肝白？为什么我一开始就要冒着风险去交易？


能不能再简单一点？有没有免费的？
​

有的，空投。
​

空投，在《绝地求生》游戏中，是给面向所有参加玩家，空中投放高级枪支道具，鼓励玩家争抢，激发活跃，加快游戏进度。


多数 NFT 项目也是类似，只不过他们空投对象是加密圈的活跃者，你理解成为「托」也好，「 KOL」也好，「投机者」也好，总之，获得空投的人，成为利益维护者，社区建造者，为项目方摇旗呐喊，  提高 NFT 的价格，吸引更多人过来过来购买。


一般来讲，Web3 新手是没有空投资格的，但事实上，Web3 新手是可以拥有空投资格的。
​

只不过这件事太简单了，太违背那些 Web3 老手的常识——这种吃力不赚钱的事情，谁去做呀。
​

但，又有何不可呢？
​

于是，我就发起了 Your First NFT 项目，算是回顾自己当初进入 Web3 困惑的回响行动。
​

花点时间设计了一张 NFT，直接铸造 10000 张，免费手工空投给所有 Web3 人。
​

当我想做一件事，就是要用最笨的方法，也要去完成它。



-------


什么是 Web3 ，它就是伊萨卡岛

```
卡瓦菲斯：伊萨卡岛



当你启程前往伊萨卡

但愿你的道路漫长，

充满奇迹，充满发现。

莱斯特律戈涅斯巨人，独眼巨人，

愤怒的波塞冬海神——不要怕他们：

你将不会在途中碰到诸如此类的怪物，

只要你高扬你的思想，

只要有一种特殊的感觉，

接触你的精神和肉体。

莱斯特律戈涅斯巨人，独眼巨人，

野蛮的波塞冬海神——你将不会跟他们遭遇

除非你将他们一直带进你的灵魂，

除非你的灵魂将他们树立在你的面前。



但愿你的道路漫长。

但愿那里有很多夏天的早晨，

当你无比快乐和兴奋地

进入你第一次见到的海港：

但愿你在腓尼基人的贸易市场停步

购买精美的物件，

珍珠母和珊瑚，琥珀和黑檀，

各式各样销魂的香水

——你要多销魂就有多销魂：



愿你走访众多埃及城市

向那些有识之士讨教并继续讨教。

让伊萨卡常在你心中，

抵达那里是你此行的目的。

但路上不要过于匆促，

最好多延长几年，

那时当你上得了岛你也就老了，

一路所得已经教你富甲四方，

用不着伊萨卡来让你财源滚滚。

用伊萨卡赋予你如此神奇的旅行，

没有它你可不会启程前来。

现在它再也没有什么可以给你的了。

而如果你发现它原来是这么穷，

那可不是伊萨卡想愚弄你。

既然那时你已经变得很聪慧，并且见多识广，

你也就不会不明白，这些伊萨卡意味着什么。
```

## Changelog

- 2022-03-21 新增官网，可免费 Mint 
- 2022-03-14 增补介绍
- 2022-02-17 增补源起
- 2022-02-16 增补路线描述

